...
class SarafAccountRegisterView(APIView):
    ...
    def post(self, request):
        ...
        # Check for existing verified accounts (outside transaction for early return)
        if SarafAccount.objects.filter(email=email, is_email_verified=True).exists():
            return Response({'error': 'Email already exists and is verified.'}, status=status.HTTP_400_BAD_REQUEST)
        
        if SarafAccount.objects.filter(email_or_whatsapp_number=email_or_whatsapp_number, is_email_verified=True).exists():
            return Response({'error': 'WhatsApp number already exists and is verified.'}, status=status.HTTP_400_BAD_REQUEST)

        # License number uniqueness check (only if provided) - only check verified accounts
        if license_no:
            if SarafAccount.objects.filter(license_no=license_no, is_email_verified=True).exists():
                return Response({'error': 'License number already exists.'}, status=status.HTTP_400_BAD_REQUEST)

        with transaction.atomic():
            # Delete any existing unverified accounts inside transaction to prevent race conditions
            # Also free up their AmuPay codes so they can be reused
            unverified_accounts = SarafAccount.objects.filter(
                Q(email=email, is_email_verified=False) |
                Q(email_or_whatsapp_number=email_or_whatsapp_number, is_email_verified=False)
            )
            
            # Free up AmuPay codes before deleting accounts
            from saraf_account.models import AmuPayCode
            for account in unverified_accounts:
                if account.amu_pay_code:
                    try:
                        code_obj = AmuPayCode.objects.get(
                            code=account.amu_pay_code.upper(),
                            is_used=True,
                            used_by=account
                        )
                        # Reset the code to unused so it can be reused
                        code_obj.is_used = False
                        code_obj.used_by = None
                        code_obj.used_at = None
                        code_obj.save(update_fields=['is_used', 'used_by', 'used_at'])
                    except AmuPayCode.DoesNotExist:
                        pass  # Code might have been deleted or doesn't exist
            
            # Now delete the unverified accounts
            ...
            saraf = SarafAccount(
                ...
                is_active=False,  # Always set to False until OTP is verified
            ...
            try:
                ...
                # If OTP sending fails, delete the account to allow retry
                saraf.delete()
                return Response({
                    'error': 'Failed to send OTP. Please try again.'
                }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        # Don't return tokens until OTP is verified - account is not active yet
        return Response({
    ...
    def post(self, request):
        try:
            ...
            if otp:
                ...
                from rest_framework_simplejwt.tokens import RefreshToken
                refresh = RefreshToken()
                refresh['saraf_id'] = saraf_account.saraf_id
                refresh['user_type'] = 'saraf'
                refresh['user_id'] = saraf_account.saraf_id
                refresh['full_name'] = saraf_account.full_name
                refresh['exchange_name'] = saraf_account.exchange_name
                refresh['email_or_whatsapp_number'] = saraf_account.email_or_whatsapp_number
