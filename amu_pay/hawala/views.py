...
class SendHawalaView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                self._update_saraf_balance(saraf_account, hawala.currency, hawala.amount, 'deposit', 'hawala_send')
    ...
    def _update_saraf_balance(self, saraf_account, currency, amount, transaction_type, description):
        """Helper method to update saraf balance"""
        try:
            from saraf_balance.models import SarafBalance
            
            # Get or create balance record
            balance, created = SarafBalance.objects.get_or_create(
            ...
            # Update balance
            balance.update_balance(amount, transaction_type)
            ...
            Transaction.objects.create(
                ...
                amount=amount,
                currency=currency,
                performer_saraf=saraf_account,
                performer_employee=None,  # Will be set if employee performed the action
                description=f"Hawala transaction: {description}"
            )
            
        except Exception as e:
            # Log error but don't fail the hawala transaction
            import logging
    ...
    def patch(self, request, hawala_number):
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                self._update_saraf_balance(saraf_account, hawala.currency, hawala.amount, 'withdrawal', 'hawala_receive')
